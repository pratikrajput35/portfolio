  ...
  return (
    <motion.div ref={cardRef} variants={cardVariants} className="break-inside-avoid pb-10" onHoverStart={() => setHovered(true)} onHoverEnd={() => setHovered(false)}>
      <TiltCard>
        ...
        >
          <div className={`relative overflow-hidden bg-[var(--card)] rounded-[2rem] border border-[var(--border)] mb-6 transition-all duration-500 hover:border-white/20 hover:shadow-2xl hover:shadow-orange-500/10 ${isMobile && hovered ? 'border-white/20 shadow-2xl shadow-orange-500/10' : ''} ${isShort ? 'aspect-[9/16]' : ''}`}>
            ...
            <div className={`transition-opacity duration-700 h-full ${hovered && (hoverVideoSrc || hasMultipleImages || project.videoProvider === 'youtube') ? 'opacity-0' : 'opacity-100'}`}>
              {project.coverImage ? (
                <Image
                  ...
                  className={`w-full h-auto object-cover group-hover:scale-105 transition-all duration-700 ease-out ${isShort ? 'h-full' : ''}`}
                />
              ) : (
                <div className={`w-full aspect-[4/3] flex items-center justify-center text-[var(--muted)] text-sm tracking-widest uppercase glass-light ${isShort ? 'h-full' : ''}`}>
                  Media
                </div>
              )}
            </div>
