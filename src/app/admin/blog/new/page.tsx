...
export default function NewBlogPostPage() {
  ...
  const handleSave = async () => {
    ...
    const res = await fetch('/api/blog', {
      ...
      body: JSON.stringify({ ...form, slug: form.slug || generateSlug(form.title), tags: form.tags.split(',').map(t => t.trim()).filter(Boolean) }),
    });
        ...
        <div className="space-y-4">
          <div className="bg-[#111] border border-gray-800 rounded-2xl p-6 space-y-4">
            ...
            <button onClick={handleSave} disabled={saving}
              ...
              {saving ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <><FiSave /> Publish Post</>}
            </button>
          </div>
        </div>
      </div>
