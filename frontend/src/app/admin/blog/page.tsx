...
export default function AdminBlogPage() {
  ...
  const fetchPosts = () => {
    setLoading(true);
    api('/api/blog?all=true').then(r => r.json()).then(d => { setPosts(Array.isArray(d) ? d : []); setLoading(false); }).catch(() => setLoading(false));
  };

  useEffect(fetchPosts, []);

  const togglePublish = async (post: any) => {
    await api(`/api/blog/${post.slug}`, {
      method: 'PUT',
      json: { ...post, isPublished: !post.isPublished, publishedAt: !post.isPublished ? new Date() : post.publishedAt },
    });
    fetchPosts();
  };
  ...
  const handleDelete = async (slug: string) => {
    ...
    await api(`/api/blog/${slug}`, { method: 'DELETE' });
