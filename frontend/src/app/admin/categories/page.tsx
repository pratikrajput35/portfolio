...
export default function AdminCategoriesPage() {
  ...
  const fetchCategories = () => {
    setLoading(true);
    api('/api/categories').then(r => r.json()).then(d => { setCategories(Array.isArray(d) ? d : []); setLoading(false); }).catch(() => setLoading(false));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await api(url, { method, json: { ...form, slug: slugToUse } });
      if (res.ok) {
